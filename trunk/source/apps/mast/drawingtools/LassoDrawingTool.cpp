...
//#################### PUBLIC METHODS ####################
void LassoDrawingTool::mouse_dragged(const Vector2i& p_Pixels, const Vector2i& position)
{
	m_drawnLocations.push_back(std::make_pair(p_Pixels, position));
}

void LassoDrawingTool::mouse_pressed(const Vector2i& p_Pixels, const Vector2i& position)
{
	m_drawnLocations.push_back(std::make_pair(p_Pixels, position));
}
