/***
 * millipede: JSpleenIdentifier3D.cpp
 * Jess Pumphrey, 2012
 ***/
...
//#################### CONSTRUCTORS ####################
JSpleenIdentifier3D::JSpleenIdentifier3D(const DICOMVolume_CPtr& dicomVolume, const VolumeIPF_Ptr& volumeIPF)
:	FeatureIdentifier(dicomVolume, volumeIPF)
{}

//#################### PUBLIC METHODS ####################
int JSpleenIdentifier3D::length() const
{
	return 1;
}

//#################### PRIVATE METHODS ####################
void JSpleenIdentifier3D::execute_impl()
{
	set_status("Identifying spleen...");
	std::cout << "Identifying spleen" << std::endl;

	VolumeIPFMultiFeatureSelection_Ptr multiFeatureSelection = get_multi_feature_selection();

	// Step 1: Filter for spleen candidates.
	std::list<PFNodeID> candidates = filter_branch_nodes(boost::bind(&JSpleenIdentifier3D::is_candidate, this, _1, _2));
...
bool JSpleenIdentifier3D::is_candidate(const PFNodeID& node, const BranchProperties& properties) const
{
