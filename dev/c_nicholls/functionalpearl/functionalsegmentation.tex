...
Due to the use of the above artifice, the overall running time of the
MST construction can be brought to the same order of magnitude as the
running time of the rest of the algorithm.
...
\centering
\ifpdf
        \epsfig{file=cases.pdf, width=.7\linewidth}%
\else
        % TODO
\fi
...
see that the function {\tt mark} acts as a tree-{\tt map} and {\tt
  elide} acts as a {\tt fold}.
...
Next, see whether {\tt f.case3and4} can be simplified.
...
\centering
\ifpdf
        \epsfig{file=ExampleOfRegionalMinima.pdf, width=.7\linewidth}%
\else
        % TODO
\fi
...
\indent
  ...
  edge from the current vertex until you @@@ the smallest edge is
