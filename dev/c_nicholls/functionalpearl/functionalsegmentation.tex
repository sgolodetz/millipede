...
\section{Motivation}
 ...
 algorithms -- proposed for the generic segmentation problem -- have
 been shown to be effective for the purpose of CT
...
algorithm which, through introducing a new data structure, simplifies
the process and improves efficiency compared to existing
...
This also means that the algorithm can be written as a single
recursive pass of the MST and hence lends itself to being expressed in
...
\item Precisely one edge from each node will satisfy case 1 or case
  2. All other nodes must be an instance of case 3 or case 4.

\item If $e$ satisfies case 2 or case 4 then we have identified the
  minimum edge of $c$ already so none of $c$'s children can satisfy
  cases 1 or 2.

\item For cases 1,2 and 4 node $p$ and $c$ will be merged and only in
  case 3 will they remain unmerged.

\end{enumerate}

For cases 1,2 and 4 node $p$ and $c$ will be merged and only in case 3
...
We define two functions of the same type, ({\tt Tree a -> Tree a}) The
first function operates on a parent node and a list of (some of) its
children and we assume the minimum child is in that list.  The second
operates on a node and a list of children without,...
...
that the minimum child will satisfy either case 1 or case 2 and the
other nodes will satisfy either case 3 or case 4.
...
If $s$ is a case 2 edge, then all of its children must be case 3 or case 4 edges.
...
\item We also require that no two edges are of equal weight,
  ...
  b_2$. This restriction @@@ provide details.
