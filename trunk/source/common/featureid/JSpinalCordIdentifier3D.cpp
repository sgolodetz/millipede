/***
 * millipede: JSpinalCordIdentifier3D.cpp
 * Jess Pumphrey, 2012
 ***/
...
//#################### CONSTRUCTORS ####################
JSpinalCordIdentifier3D::JSpinalCordIdentifier3D(const DICOMVolume_CPtr& dicomVolume, const VolumeIPF_Ptr& volumeIPF)
:	FeatureIdentifier(dicomVolume, volumeIPF)
{}

//#################### PUBLIC METHODS ####################
int JSpinalCordIdentifier3D::length() const
{
	return 1;
}

//#################### PRIVATE METHODS ####################
void JSpinalCordIdentifier3D::execute_impl()
{
	set_status("Identifying spinal cord...");
	std::cout << "Identifying spinal cord" << std::endl;
	...
	std::list<PFNodeID> nodes = filter_branch_nodes(boost::bind(&JSpinalCordIdentifier3D::is_spinal_cord, this, _1, _2, spineProperties));
...
bool JSpinalCordIdentifier3D::is_spinal_cord(const PFNodeID& node, const BranchProperties& properties, const BranchProperties& spineProperties) const
{
