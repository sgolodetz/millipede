/***
 * millipede: JSpineIdentifier3D.cpp
 * Jess Pumphrey, 2012
 ***/
...
//#################### CONSTRUCTORS ####################
JSpineIdentifier3D::JSpineIdentifier3D(const DICOMVolume_CPtr& dicomVolume, const VolumeIPF_Ptr& volumeIPF)
:	FeatureIdentifier(dicomVolume, volumeIPF)
{}

//#################### PUBLIC METHODS ####################
int JSpineIdentifier3D::length() const
{
	return 1;
}

//#################### PRIVATE METHODS ####################
void JSpineIdentifier3D::execute_impl()
{
	set_status("Identifying the spine...");
	std::cout << "Identifying spine" << std::endl;

	VolumeIPFMultiFeatureSelection_Ptr multiFeatureSelection = get_multi_feature_selection();

	std::list<PFNodeID> nodes = filter_branch_nodes(boost::bind(&JSpineIdentifier3D::is_spine, this, _1, _2));
...
bool JSpineIdentifier3D::is_spine(const PFNodeID& node, const BranchProperties& properties) const
{
