...
Some things to note:
\begin{enumerate}
\item Precisely one edge from each node will satisfy case 1 or case 2. All other nodes must be an instance of case 3 or case 4.
...
The first function operates on a parent node and a list of (some of) its children and we assume the minimum child is in that list.
The second operates on a node and a list of children without,...


The tree structure is originally defied as
...
Since the waterfall algorithm works by merging together, we will need a merge operation on each region of the graph. To do this we define a type class mergeable.
...
It now remains only to define the function $mark$. This function starts at the root of an unlabelled tree and walks through, labelling each edge. The only edges of the root are its children, so we know the minimum edge of the root is its smallest child. By lemma 1, we know that the minimum child will satisfy either case 1 or case 2 and the other nodes will satisfy either case 3 or case 4.
...
It would be good to avoid creating the intermediate LTree structure. Currently the LTree is created by mark and consumed directly by elide, so it should be possible to fuse the two functions into one pass.
...
\item We also require that no two edges are of equal weight, $(a_1,b_1,k) \in E\
 and\ (a_2,b_2,k) \implies a_1 = a_2\ and\ b_1 = b_2$. This restriction ??? provide details.
...
Define a regional-minimum to be an edge that is surrounded by edges of greater weight, i.e.
An edge $e = (a,b,k)$ is a regional minimum iff
$\forall c \in E, l \in \mathbb{N} :
  (c,b,l) \in E \implies l \geq k\ and\
  (a,c,l) \in E \implies l \geq k $
Equivalently, $e$ is the edge of smallest weight connoted to nodes $a$ and similarly for node $b$.
