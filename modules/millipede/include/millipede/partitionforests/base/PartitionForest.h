...
#include "../../listeners/CompositeListenerBase.h"
#include "IForestLayer.h"
#include "PartitionForestCut.h"

namespace mp {

/**
@brief	The different tug modes supported (see the paper "Simplifying TugGraph using Zipping Algorithms").
*/
enum TugMode
{
	/// FastTug
	...
	/// A dummy value containing the number of tug modes
	TUGMODE_COUNT
};
...
public:
	...
	*/
	...
	/**
	@brief	Jointly unzips multiple nodes in the partition forest as far up as the specified layer.

	Multi-node unzipping is a generalisation of single-node unzipping that allows several
	nodes to be jointly unzipped at the same time. The motivations for this, as opposed to
	...
	@param[in]	nodesByDepth		The nodes to be unzipped, grouped by depth
	@param[in]	toLayer				The layer to which to unzip them
	@param[in]	checkPreconditions	Whether or not the preconditions need to be explicitly checked (default: yes)
	@pre
		-	The specified nodes are all valid
		-	For each specified node, node.layer() < toLayer <= highest_layer()
	@post
		-	The partition forest will have been consistently modified so as to unzip the nodes to the specified layer
	@throw Exception
		-	If the preconditions are violated
	@return The node chains generated by the unzip
	*/
	std::map<PFNodeID,Chain> unzip_nodes(const std::map<int,std::set<PFNodeID> >& nodesByDepth, int toLayer,
	                                     UnzipMode unzipMode = UNZIPMODE_DEFAULT, CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		return unzip_nodes(nodesByDepth, PartitionForestCut_CPtr(new HorizontalCut(toLayer)), unzipMode, checkPreconditions);
	}

	/**
	@brief	Jointly unzips multiple nodes in the partition forest as far up as the specified cut.

	Multi-node unzipping up to a cut is a generalisation of multi-node unzipping up to a layer
	(which is just a horizontal cut). The motivation for supporting this is to allow TugGraph
	to be reformulated as an unzip of the leaves adjacent to the node being tugged up to a cut.
	For more details, see the paper "Simplifying TugGraph using Zipping Algorithms".

	@note	This method executes a command sequence (which can be atomically undone, if an UndoableCommandManager
			has been previously installed using set_command_manager()).

	@param[in]  nodesByDepth		The nodes to be unzipped, grouped by depth
	@param[in]	cut					The cut to which to unzip them
	@param[in]	checkPreconditions	Whether or not the preconditions need to be explicitly checked (default: yes)
	@pre
		-	The specified nodes are all valid
		-	The cut is above every specified node
	@post
		-	The partition forest will have been consistently modified so as to unzip the nodes to the specified cut
	@throw Exception
		-	If the preconditions are violated
	@return	The node chains generated by the unzip
	*/
	std::map<PFNodeID,Chain> unzip_nodes(const std::map<int,std::set<PFNodeID> >& nodesByDepth, const PartitionForestCut_CPtr& cut,
	                                     UnzipMode unzipMode = UNZIPMODE_DEFAULT, CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		if(checkPreconditions)
		{
			// Check that all of the specified nodes are valid and that the cut is above every node.
			for(std::map<int,std::set<PFNodeID> >::const_iterator it = nodesByDepth.begin(), iend = nodesByDepth.end(); it != iend; ++it)
			{
				for(std::set<PFNodeID>::const_iterator jt = it->second.begin(), jend = it->second.end(); jt != jend; ++jt)
				{
					...
					// Check that the node is below the cut.
					while(node != PFNodeID::invalid() && !cut->contains(node)) node = parent_of(node);
					if(node == PFNodeID::invalid()) throw Exception(OSSWrapper() << "Invalid cut");
				}
			}
		}
		...
		// Unzip the nodes, starting with those deepest in the hierarchy.
		int layer = nodesByDepth.begin()->first;
		std::set<PFNodeID> curs = nodesByDepth.begin()->second;
		while(layer < cut->max_layer())
		{
			    ...
			    it != iend; ++it)
			{
				...
				// If the parent node is on the cut, remove all of its selected children from the list of current nodes.
				if(cut->contains(parent))
				{
					for(std::set<int>::const_iterator jt = selectedChildren.begin(), jend = selectedChildren.end(); jt != jend; ++jt)
					{
						curs.erase(PFNodeID(layer,*jt));
					}
				}
				...
				// Calculate the connected components of the selected children.
				...
				// Depending on the unzip mode, add to these either the connected components of the unselected children,
				// or a single (potentially unconnected) component containing the unselected children (if any).
				if(unzipMode == UNZIPMODE_DEFAULT)
				{
				...
				else if(!unselectedChildren.empty())
				{
					connectedComponents.push_back(unselectedChildren);
				}
	...
	*/
	...
	/**
	@brief	Floods the specified node down to the specified layer.

	In practical terms, this means removing all the hierarchical structure between the node
	and its descendants in the specified layer.

	@note	This method executes a command sequence (which can be atomically undone, if an UndoableCommandManager
			has been previously installed using set_command_manager()).

	@param[in]	node				The node to be flooded
	@param[in]	toLayer				The layer down to which to flood it
	@param[in]	checkPreconditions	Whether or not the preconditions need to be explicitly checked (default: yes)
	@pre
		-	The node to be flooded is valid
		-	0 <= toLayer < node.layer()
	@post
		-	The partition forest will have been consistently modified so as to flood the specified node
			down to the specified layer
	@throw Exception
		-	If the preconditions are violated
	*/
	void flood_node(const PFNodeID& node, int toLayer, CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		if(checkPreconditions)
		{
			// Check that the specified node is valid.
			...
			// Check that the specified layer is within the right range.
			if(toLayer < 0 || toLayer >= node.layer()) throw Exception(OSSWrapper() << "Invalid layer: " << toLayer);
		}

		SequenceGuard guard(m_commandManager, "Flood Node", m_listeners);

		std::list<PFNodeID> descendants = descendants_in_layer(node, toLayer);
		for(std::list<PFNodeID>::const_iterator it = descendants.begin(), iend = descendants.end(); it != iend; ++it)
		{
			unzip_node(*it, node.layer() - 1, DONT_CHECK_PRECONDITIONS);
		}
	}
	...
	*/
	...
	/**
	@brief  Uses the FastTug approach to separate the 'atomic' nodes adjacent to the specified node out of their
			ancestors below the specified cut to allow the neighbourhood of the specified node to be visualised.

	The algorithm is a reformulation for our context of the one described in the paper "Efficiently Modifying Path-Preserving
	Hierarchies for Browsing Paths". The reformulation is described in the paper "Simplifying TugGraph using Zipping Algorithms".

	@param[in]	node				The node being tugged, i.e. whose adjacent atomics are to be separated out from their ancestors
	@param[in]	atomicLayer			The index of the atomic layer
	@param[in]	cut					The cut up to which the adjacent atomics should be separated out
	@param[in]	checkPreconditions	Whether or not the preconditions need to be explicitly checked (default: yes)
	@throw Exception
		-	If the preconditions are violated
	*/
	void tug_fast(const PFNodeID& node, int atomicLayer, const PartitionForestCut_CPtr& cut, CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		if(checkPreconditions)
		{
			// TODO: We are not yet checking any preconditions.
		}

		// Find the nodes in the atomic layer that are adjacent to those subsumed by the selected node (the 'adjacent atomics').
		std::set<int> adjAtomics = find_adjacent_atomics(node, atomicLayer);

		SequenceGuard guard(m_commandManager, "Tug Node", m_listeners);

		// Unzip these nodes up to the cut, ripping their ancestors if necessary in the process.
		std::map<int,std::set<PFNodeID> > adjAtomicsByDepth;
		for(std::set<int>::const_iterator it = adjAtomics.begin(), iend = adjAtomics.end(); it != iend; ++it)
		{
			adjAtomicsByDepth[atomicLayer].insert(PFNodeID(atomicLayer, *it));
		}

		std::map<PFNodeID,Chain> chains = unzip_nodes(adjAtomicsByDepth, cut, UNZIPMODE_RIP, DONT_CHECK_PRECONDITIONS);

		// Determine which nodes might have been ripped, ordered by layer (lowest first).
		std::map<int,std::set<PFNodeID> > maybeRippedNodes;
		for(std::map<PFNodeID,Chain>::const_iterator it = chains.begin(), iend = chains.end(); it != iend; ++it)
		{
			for(Chain::const_iterator jt = it->second.begin(), jend = it->second.end(); jt != jend; ++jt)
			{
				maybeRippedNodes[jt->layer()].insert(*jt);
			}
		}

		// Fix up the nodes in order.
		for(std::map<int,std::set<PFNodeID> >::const_iterator it = maybeRippedNodes.begin(), iend = maybeRippedNodes.end(); it != iend; ++it)
		{
			const int childLayer = it->first - 1;

			for(std::set<PFNodeID>::const_iterator jt = it->second.begin(), jend = it->second.end(); jt != jend; ++jt)
			{
				std::set<PFNodeID> children = children_of(*jt);

				std::set<int> childIndices;
				for(std::set<PFNodeID>::const_iterator kt = children.begin(), kend = children.end(); kt != kend; ++kt)
				{
					childIndices.insert(kt->index());
				}

				std::vector<std::set<int> > ccs = find_connected_components(childIndices, childLayer);

				split_node(*jt, ccs, DONT_CHECK_PRECONDITIONS);
			}
		}
	}

	/**
	@brief  Separates the 'atomic' nodes adjacent to the specified node out of their ancestors below the
			specified cut to allow the neighbourhood of the specified node to be visualised.

	Two different versions of this are implemented - FastTug and SimpleTug. The differences between
	them are described in the paper "Simplifying TugGraph using Zipping Algorithms".

	@param[in]	node				The node being tugged, i.e. whose adjacent atomics are to be separated out from their ancestors
	@param[in]	atomicLayer			The index of the atomic layer
	@param[in]	cut					The cut up to which the adjacent atomics should be separated out
	@param[in]	mode				The tug mode (i.e. whether to use FastTug or SimpleTug)
	@param[in]	checkPreconditions	Whether or not the preconditions need to be explicitly checked (default: yes)
	@throw Exception
		-	If the preconditions are violated
	*/
	void tug_node(const PFNodeID& node, int atomicLayer, const PartitionForestCut_CPtr& cut, TugMode mode,
	              CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		switch(mode)
		{
			case TUGMODE_FAST:
				tug_fast(node, atomicLayer, cut, checkPreconditions);
				break;
			case TUGMODE_SIMPLE:
				tug_simple(node, atomicLayer, cut, checkPreconditions);
				break;
			default:
				throw std::runtime_error("Error: Unknown tug mode");
		}
	}

	/**
	@brief  Uses the SimpleTug approach to separate the 'atomic' nodes adjacent to the specified node out of their
			ancestors below the specified cut to allow the neighbourhood of the specified node to be visualised.

	The algorithm is described in the paper "Simplifying TugGraph using Zipping Algorithms".
	...
	@throw Exception
		-	If the preconditions are violated
	*/
	void tug_simple(const PFNodeID& node, int atomicLayer, const PartitionForestCut_CPtr& cut, CheckPreconditions checkPreconditions = CHECK_PRECONDITIONS)
	{
		if(checkPreconditions)
		{
			// TODO: We are not yet checking any preconditions.
		...
		// Unzip these nodes up to the cut.
		std::map<int,std::set<PFNodeID> > adjAtomicsByDepth;
		for(std::set<int>::const_iterator it = adjAtomics.begin(), iend = adjAtomics.end(); it != iend; ++it)
		{
			adjAtomicsByDepth[atomicLayer].insert(PFNodeID(atomicLayer, *it));
		...
		SequenceGuard guard(m_commandManager, "Tug Node", m_listeners);
		unzip_nodes(adjAtomicsByDepth, cut, UNZIPMODE_DEFAULT, DONT_CHECK_PRECONDITIONS);
	}
...
private:
	...
	/**
	@brief  Find the nodes in the specified atomic layer that are adjacent to those subsumed by the specified node.

	@param[in]	node		The node whose adjacent atomics we want to find
	@param[in]	atomicLayer	The index of the atomic layer
	@pre
		-	1 <= atomicLayer <= highest_layer()
	@return	The nodes in the atomic layer adjacent to those subsumed by the node
	*/
	std::set<int> find_adjacent_atomics(const PFNodeID& node, int atomicLayer) const
	{
		// Step 1: Traverse down the hierarchy from the node to find the atomic nodes it subsumes.
		std::deque<int> receptiveRegion = receptive_region_of(node);
		std::set<PFNodeID> receptiveAtomics;
		for(std::deque<int>::const_iterator it = receptiveRegion.begin(), iend = receptiveRegion.end(); it != iend; ++it)
		{
			receptiveAtomics.insert(ancestor_of(PFNodeID(0,*it), atomicLayer));
		}

		// Step 2: Find the atomics adjacent to those atomics (the 'adjacent atomics').
		std::set<int> adjAtomics;
		for(std::set<PFNodeID>::const_iterator it = receptiveAtomics.begin(), iend = receptiveAtomics.end(); it != iend; ++it)
		{
			std::vector<int> temp = adjacent_nodes(*it);
			std::copy(temp.begin(), temp.end(), std::inserter(adjAtomics, adjAtomics.begin()));
		}

		for(std::set<PFNodeID>::const_iterator it = receptiveAtomics.begin(), iend = receptiveAtomics.end(); it != iend; ++it)
		{
			adjAtomics.erase(it->index());
		}
